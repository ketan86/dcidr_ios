...
class EventDetailViewController: BaseViewController {
    ...
    @IBOutlet weak var mEventDetailScrollView: UIScrollView!
    @IBOutlet weak var mPageTitle: UINavigationItem!
    @IBOutlet weak var mGroupName: UILabel!
    @IBOutlet weak var mEventEndTime: UILabel!
    @IBOutlet weak var mEventStartTime: UILabel!
    ...
    @IBOutlet weak var mGroupInfoHolderView: UIView!
    @IBOutlet weak var mEventInfoHolderView: UIView!
    ...
    @IBAction func onRecreateButtonClicked(_ sender: UIButton) {
        ...
        newBaseEventViewCtr.mCaller = "NewBaseEventViewController"
        newBaseEventViewCtr .mGroupId = self.mGroupId!
        newBaseEventViewCtr.mEventId = self.mEventId!
        newBaseEventViewCtr.mEventType = self.mBaseEvent!.getEventTypeStr()
    ...
    override func initViewController() {
        ...
        if (self.mGroupId != nil) {
            if (self.mEventId != nil) {
                let baseGroup: BaseGroup = DcidrApplication.getInstance().getGlobalHistoryContainer().getBaseGroup(self.mGroupId!)!
                self.mBaseEvent = baseGroup.getEventContainer().getBaseEvent(self.mEventId!)
                self.mPageTitle.title = "Activity Details"
                if(self.mBaseEvent!.getEventProfilePicData() == nil) {
                    self.mBaseEvent!.seEventProfilePicFetchDoneCb({ (imageData : Data) in
                        self.mEventProfilePic.image = UIImage(data: imageData)!
                    })
                }else {
                    if let data = self.mBaseEvent!.getEventProfilePicData() {
                        self.mEventProfilePic.image = UIImage(data: data)!
                    }
                }
                
                self.mEventName.text = self.mBaseEvent!.getEventName()
                self.mEventPlace.text = self.mBaseEvent!.getLocationName()
                self.mEventStartTime.text = Utils.epochToDate(epoch: self.mBaseEvent!.getStartTime())
                self.mEventEndTime.text  = Utils.epochToDate(epoch: self.mBaseEvent!.getEndTime())
                
                self.mGroupName.text = self.mBaseEvent!.getBaseGroup()!.groupName
    ...
    func createMembersHolderStackView() {
        ...
        self.mEventDetailScrollView.contentSize = CGSize(width: CGFloat(self.mEventDetailScrollView.frame.width), height: CGFloat(self.mMembersHolderStackView.subviews.count * 50) + CGFloat(self.mEventProfilePic.frame.height) + CGFloat(self.mEventInfoHolderView.frame.height) + CGFloat(self.mGroupInfoHolderView.frame.height) + CGFloat(self.mMembersHolderView.frame.height) + 10)
    }
